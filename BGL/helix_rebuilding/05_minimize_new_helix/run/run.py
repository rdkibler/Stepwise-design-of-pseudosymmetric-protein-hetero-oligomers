...
hbnet_pdb = input_pdb[:-11].split("/")[-1] + ".pdb"
hbnet_pdb_path = ""
...
loop_closure_type = name.split("_")[-4]
if loop_closure_type == "cl1":
	sele_from="-1,L"
	sele_to="c_term"
	sele_chain="A"
elif loop_closure_type == "cl2":
	sele_from="n_term"
	sele_to="1,L"
	sele_chain="B"
else:
	exit("BAD LOOP_CLOSURE_TYPE:", loop_closure_type)
...
with tempfile.NamedTemporaryFile() as temp:
	...
	command=f"/home/bcov/ppi/builds/19-09-18/main/source/cmake/build_release_omp_hdf5/rosetta_scripts " \
		...
		f"-parser:script_vars cst_file={temp.name} aacomp={aa_comp} sele_from={sele_from} sele_to={sele_to} sele_chain={sele_chain} " \
		f"-nstruct 1 " \
		f"-overwrite 1 " \
